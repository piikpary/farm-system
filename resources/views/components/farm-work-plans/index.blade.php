    ...
    private function restoreFuelStock(float $restoreLiters, string $note = null): void
{
    ...
    public function saveRow($index)
    {
        ...
        DB::transaction(function () use ($row, $zoneBlockIds, $requestLiters) {
            ...
            $this->deductFuelStock(
                (float) $requestLiters,
                'Fuel requested from work plan #' . $plan->id
            );
        });
    ...
    public function updateRow()
    {
        ...
        DB::transaction(function () use ($plan, $zoneBlockIds, $requestLiters) {
            $oldRequestLiters = (float) $plan->request_liters;
            $newRequestLiters = (float) $requestLiters;
            $difference = $newRequestLiters - $oldRequestLiters;
            ...
            $plan->update([
                ...
                'request_liters' => $newRequestLiters,
            ...
            if ($difference > 0) {
                $this->deductFuelStock(
                    $difference,
                    'Additional fuel requested from work plan #' . $plan->id
                );
            }

            if ($difference < 0) {
                $this->restoreFuelStock(
                    abs($difference),
                    'Fuel restored from work plan #' . $plan->id
                );
            }
        });
    ...
    public function delete($id)
    {
        ...
        DB::transaction(function () use ($id) {
            ...
            $this->restoreFuelStock(
                ...
                'Fuel restored after deleting work plan #' . $plan->id
            );
    ...
    {
        return FarmWorkPlan::with('taskCategory')
    ...
    public function exportWorkPlansExcel()
{
    ...
    $headers = [
        ...
        'B1' => __('pages.activity'),
        'C1' => __('pages.plan_start'),
        'D1' => __('pages.plan_end'),
        'E1' => __('pages.zone_block'),
        'F1' => __('pages.plan_area_ha'),
        'G1' => __('pages.request_l_ha'),
        'H1' => __('pages.request_l'),
        'I1' => __('pages.status'),
    ...
    foreach ($plans as $plan) {
        $sheet->setCellValue('A' . $rowNumber, optional($plan->plan_date)->format('Y-m-d'));
        $sheet->setCellValue('B' . $rowNumber, $plan->taskCategory->name ?? '-');
        $sheet->setCellValue('C' . $rowNumber, optional($plan->plan_start)->format('Y-m-d'));
        $sheet->setCellValue('D' . $rowNumber, optional($plan->plan_end)->format('Y-m-d'));
        $sheet->setCellValue('E' . $rowNumber, $this->getZoneBlockLabel($plan->zone_block_ids));

        // Numeric cells
        $sheet->setCellValue('F' . $rowNumber, (float) $plan->plan_area);
        $sheet->setCellValue('G' . $rowNumber, (float) $plan->request_l_per_hectare);

        // Formula: Request L = Plan Area * Request L / HA
        $sheet->setCellValue('H' . $rowNumber, '=F' . $rowNumber . '*G' . $rowNumber);

        $sheet->setCellValue('I' . $rowNumber, __('pages.' . $plan->status));
    ...
    // Total row
    if ($lastDataRow >= 2) {
        $sheet->setCellValue('E' . $rowNumber, __('pages.total_plans') . ': ' . $plans->count());
    ...
    $sheet->getStyle('F2:H' . max($rowNumber, 2))
        ->getNumberFormat()
        ->setFormatCode('#,##0.00');
    ...
    public function getSelectedZoneBlockBadges($ids)
    {
        ...
        return ZoneBlock::with('zone')
            ...
            ->map(function ($block) {
                return [
                    'id' => $block->id,
                    'label' => (optional($block->zone)->zone_code ? optional($block->zone)->zone_code . ' / ' : '') . $block->block_code,
                ];
            });
    }
...
<div class="page">
    ...
    <style>
        ...
        .plan-table input, .plan-table select { width:100%; min-width:130px; height:42px; padding:8px 10px; border:1px solid #d1d5db; border-radius:10px; font-size:13px; background:#ffffff; font-weight:700; }
        ...
        .zone-modal-backdrop { position:fixed; inset:0; z-index:9998; background:rgba(15, 23, 42, 0.45); display:flex; align-items:center; justify-content:center; padding:24px; }
        .zone-modal { width:min(900px, 96vw); max-height:86vh; background:#ffffff; border-radius:22px; box-shadow:0 30px 80px rgba(15, 23, 42, 0.35); overflow:hidden; }
        .zone-modal-header { display:flex; align-items:center; justify-content:space-between; gap:14px; padding:18px 22px; border-bottom:1px solid #e5e7eb; background:#f8fafc; }
        .zone-modal-title { font-size:18px; font-weight:900; color:#0f172a; }
        .zone-modal-subtitle { margin-top:3px; font-size:13px; font-weight:700; color:#64748b; }
        .zone-modal-actions { display:flex; gap:8px; align-items:center; }
        .zone-modal-body { padding:18px 22px; max-height:55vh; overflow-y:auto; }
        .zone-grid { display:grid; grid-template-columns:repeat(3, minmax(0, 1fr)); gap:12px; }
        .zone-card { display:flex; align-items:flex-start; gap:10px; padding:13px; border:1px solid #e5e7eb; border-radius:16px; cursor:pointer; background:#ffffff; transition:0.15s ease; }
        .zone-card:hover { border-color:#86efac; background:#f0fdf4; }
        .zone-card input { width:18px !important; min-width:18px !important; height:18px !important; margin-top:2px; accent-color:#16a34a; }
        .zone-card-main { font-size:14px; font-weight:900; color:#0f172a; }
        .zone-card-sub { margin-top:3px; font-size:12px; font-weight:700; color:#64748b; }
        .zone-modal-footer { display:flex; align-items:center; justify-content:space-between; gap:12px; padding:14px 22px; border-top:1px solid #e5e7eb; background:#ffffff; }
        .zone-selected-preview { display:flex; flex-wrap:wrap; gap:6px; max-width:520px; }
        .zone-badge { display:inline-flex; align-items:center; padding:5px 9px; border-radius:999px; background:#dcfce7; color:#166534; font-size:11px; font-weight:900; }
        ...
        @media (max-width:900px) {
            ...
            .zone-modal-header,
            .zone-modal-footer { align-items:stretch; flex-direction:column; }
        }
    </style>
            ...
            <table class="plan-table">
                ...
                <tbody>
                    ...
                    @empty
                        @if(count($rows) === 0)
                            <tr>
                                <td colspan="11" class="empty">{{ __('pages.no_work_plan_found') }}</td>
                            </tr>
                        @endif
                    @endforelse
            ...
            <div class="zone-modal">
                <div class="zone-modal-header">
                    ...
                    <div class="zone-modal-actions">
                        <button type="button" class="btn gray" wire:click="selectAllActiveZoneBlocks">{{ __('pages.select_all') }}</button>
                        <button type="button" class="btn gray" wire:click="clearActiveZoneBlocks">{{ __('pages.clear') }}</button>
                        <button type="button" class="mini danger" wire:click="closeZonePicker">×</button>
                    </div>
                </div>

                <div class="zone-modal-body">
                    <div class="zone-grid">
                        @forelse($zoneBlocks as $block)
                            <label class="zone-card">
                                @if($zonePickerMode === 'edit')
                                    <input type="checkbox" value="{{ $block->id }}" wire:model.live="editRow.zone_block_ids">
                                @else
                                    <input type="checkbox" value="{{ $block->id }}" wire:model.live="rows.{{ $zonePickerIndex }}.zone_block_ids">
                                @endif
                                ...
                                <span>
                                    ...
                                    <div class="zone-card-sub">
                                        {{ optional($block->zone)->name ?? 'Zone' }}
                                        {{ $block->name ? ' - ' . $block->name : '' }}
                                    </div>
                                </span>
                            </label>
                        @empty
                            <div class="empty" style="grid-column:1 / -1;">{{ __('pages.no_zone_block_found') }}</div>
                        @endforelse
                    </div>
                </div>
                ...
                <div class="zone-modal-footer">
                    ...
                    <div class="zone-selected-preview">
                        @foreach($this->getSelectedZoneBlockBadges($activeSelectedIds) as $badge)
